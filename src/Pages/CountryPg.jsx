...
export const CountryPg = () => {  
  ...
  return (
    <div className='countrypg'>
      <Link to="/Country" style={{zIndex:1,color:'blue',cursor:'pointer'}}><FaArrowLeftLong/></Link>
      <div className="countrypgbox">
        <img src={apiData.flags.svg} alt="flag" />
        <div className='countrydata'>
          <h2>{apiData.name.common}</h2>
          <p>Native Names: {apiData?.name?.nativeName && [
            ...new Set(Object.keys(apiData.name.nativeName).map((key) => 
              apiData.name.nativeName[key].official
            ))
          ].join(", ")}
          </p>
          <p>Region: {apiData.region}</p>
          <p>Languages: {apiData.languages && Object.keys(apiData.languages).map((item)=>apiData.languages[item])}</p>
          <p>Currency: {apiData.currencies && Object.keys(apiData.currencies).map((item)=>apiData.currencies[item].name).join(", ")}</p>
